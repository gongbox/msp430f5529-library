...
void I2C_MasterInit(I2Cn i2cn, uint16_t SlaveID, uint32_t BaudRate)
{
    ...
    {
        GPIO_WriteBit(I2C_PIN[i2cn - I2C0].SCL.Port,
                      I2C_PIN[i2cn - I2C0].SCL.Pin, BIT_SET);
        DELAY_US(5);
        GPIO_WriteBit(I2C_PIN[i2cn - I2C0].SCL.Port,
                      I2C_PIN[i2cn - I2C0].SCL.Pin, RESET);
        DELAY_US(5);
    }
    //初始化引脚
...
{
    while (USCIX[i2cn]->UC_BUSY == BIT_SET)
        ;                    //等待发送或接收完成
}
/*******************************************************************************
