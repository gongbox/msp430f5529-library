...
const TIMER_MemMapPtr TIMERX[TIMER_NUM] = { TIMER_A0_BASE_PTR,
                                            TIMER_A1_BASE_PTR,
                                            TIMER_A2_BASE_PTR,
                                            TIMER_B0_BASE_PTR };
//定义数组保存占空比最大值
const uint32_t TIMER_PRECISON[TIMER_NUM] = { TIMER_PWM_A0_PRECISON,
                                             TIMER_PWM_A1_PRECISON,
                                             TIMER_PWM_A2_PRECISON,
                                             TIMER_PWM_B0_PRECISON };
//定义通道引脚
 ...
 ********************************************************************************/
STATUS TIMER_PWM_MultiChannel_Init(TIMERn timer, uint32_t fre,
                                   uint8_t ChannelNum, ...)
{
#ifdef DELAY_TIMER
  ASSERT((timer != DELAY_TIMER),
    ...
    for (int i = 0; i < ChannelNum; i++)           //根据初始化的通道数，逐一初始化通道
    {
        ...
        //初始化端口
        GPIO_Init(TIMER_CHANEL_PIN[timer][ch].Port,
                  TIMER_CHANEL_PIN[timer][ch].Pin, GPO | SEL);
        //初始化占空比及输出波形
 ...
 ********************************************************************************/
void TIMER_PWM_SetChannelOutmod(TIMERn timer, TIMER_CHn ch,
                                TIMER_PWM_OUTMODn outmod)
{
...
void TIMER_PWM_SetChannelDuty(TIMERn timer, TIMER_CHn ch, uint32_t duty)
{
    ...
    TIMERX[timer]->CCR[ch] = (uint16_t) ((duty * (TIMERX[timer]->CCR[0] + 1))
            / TIMER_PRECISON[timer]);
}
/*******************************************************************************
...
STATUS TIMER_Capture_Init(TIMERn timer, TIMER_CHn ch, TIMER_CAPTUREn capmode)
{
  ...
  ASSERT((timer != DELAY_TIMER),
    ...
    TIMERX[timer]->CCTL[ch].CCIS =
            (timer == TIMER_B0) ? TIMER_CCIS_1 : TIMER_CCIS_0;
    ...
    //端口设置为输入方,选择第二功能
    GPIO_Init(TIMER_CHANEL_PIN[timer][ch].Port, TIMER_CHANEL_PIN[timer][ch].Pin,
              GPI | SEL);
 ...
 *  使用例程：uint16 overflowTime;  //定义变量，在定时器溢出中断里对该值增一
 ...
 overflowTime=0;       //清零
 ********************************************************************************/
uint32_t TIMER_Capture_CalTime_Us(uint16_t start_value, uint16_t stop_value,
                                  uint16_t overflowTime)
{
    return (uint32_t) ((((int32_t) stop_value - (int32_t) start_value)
            + (uint32_t) overflowTime * 65536) / g_sClock.SMCLK.fMHZ);
}
/*******************************************************************************
...
{
    return (uint32_t) ((TIMERX[timer]->R + overflowTime * 65536u)
            / g_sClock.SMCLK.fMHZ);
}
/*******************************************************************************
