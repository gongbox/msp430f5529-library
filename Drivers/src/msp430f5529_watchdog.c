...
static const uint32_t WDT_Counter[] = { 2000000000u, 128000000u, 8192000u,
                                        512000u, 32768u, 8192u, 512u, 64u };
 ...
 *  功能说明：设置看门狗时间，注意定时时间只是比较接近设置时间的一个时间，并不精准
 ...
 *  功能说明：设置看门狗作为定时器并设置时间，注意定时时间只是比较接近设置时间的一个时间，并不精准
 *  参数说明：uint32_t ms          ：功能时间
 *  函数返回：无
 ...
 *  功能说明：设置看门狗延时一段时间，注意定时时间只是比较接近设置时间的一个时间，并不精准
 *  参数说明：uint32_t ms          ：功能时间
 *  函数返回：无
...
uint16_t WatchDog_Time_Cal(uint32_t ms)
{
    ...
    int32_t smclk_OffectTimeTemp = 0, aclk_OffectTimeTemp = 0,
            vlo_OffectTimeTemp = 0;
    uint16_t smclk_DividerValue = 7, aclk_DividerValue = 7,
            vlo_DividerValue = 7;
    ...
    while ((WDT_CounterValue > WDT_Counter[smclk_DividerValue])
            && (smclk_DividerValue > 0))
    {
        smclk_DividerValue--;
    }
    if (smclk_DividerValue == 7 || smclk_DividerValue == 0)
    {
        smclk_OffectTimeTemp = (WDT_Counter[smclk_DividerValue]
                - WDT_CounterValue);
    }
    else //在两个时间内选出误差时间最短的
    {
        if ((WDT_Counter[smclk_DividerValue] - WDT_CounterValue)
                > (WDT_CounterValue - WDT_Counter[smclk_DividerValue + 1]))
        {
            smclk_OffectTimeTemp = (WDT_CounterValue
                    - WDT_Counter[++smclk_DividerValue]);
        }
        else
        {
            smclk_OffectTimeTemp = (WDT_Counter[smclk_DividerValue]
    ...
    smclk_OffectTimeTemp =
            smclk_OffectTimeTemp > 0 ?
                    smclk_OffectTimeTemp : -smclk_OffectTimeTemp;
    smclk_OffectTime = (float) smclk_OffectTimeTemp
            / (float) g_sClock.SMCLK.nHZ;
    /*ACLK*/
    SelectClock = g_sClock.ACLK.nHZ;
    WDT_CounterValue = ms * SelectClock / 1000;
    while ((WDT_CounterValue > WDT_Counter[aclk_DividerValue])
            && (aclk_DividerValue > 0))
    {
        aclk_DividerValue--;
    }
    ...
    {
        aclk_OffectTimeTemp =
                (WDT_Counter[aclk_DividerValue] - WDT_CounterValue);
    }
    else //在两个时间内选出误差时间最短的
    {
        if ((WDT_Counter[aclk_DividerValue] - WDT_CounterValue)
                > (WDT_CounterValue - WDT_Counter[aclk_DividerValue + 1]))
        {
            aclk_OffectTimeTemp = (WDT_CounterValue
                    - WDT_Counter[++aclk_DividerValue]);
        }
        else
        {
            aclk_OffectTimeTemp = (WDT_Counter[aclk_DividerValue]
                    - WDT_CounterValue);
        }
    }
    aclk_OffectTimeTemp =
            aclk_OffectTimeTemp > 0 ?
                    aclk_OffectTimeTemp : -aclk_OffectTimeTemp;
    aclk_OffectTime = (float) aclk_OffectTimeTemp / (float) g_sClock.ACLK.nHZ;
    /**VLOCLK_10K*/
    SelectClock = VLOCLK_FREQUENCY;
    WDT_CounterValue = ms * SelectClock / 1000;
    while ((WDT_CounterValue > WDT_Counter[vlo_DividerValue])
            && (vlo_DividerValue > 0))
    {
        vlo_DividerValue--;
    }
    ...
    {
        if ((WDT_Counter[vlo_DividerValue] - WDT_CounterValue)
                > (WDT_CounterValue - WDT_Counter[vlo_DividerValue + 1]))
        {
            vlo_OffectTimeTemp = (WDT_CounterValue
                    - WDT_Counter[++vlo_DividerValue]);
        }
        else
        {
            vlo_OffectTimeTemp = (WDT_Counter[vlo_DividerValue]
                    - WDT_CounterValue);
        }
    }
    vlo_OffectTimeTemp =
            vlo_OffectTimeTemp > 0 ? vlo_OffectTimeTemp : -vlo_OffectTimeTemp;
    vlo_OffectTime = (float) vlo_OffectTimeTemp / (float) VLOCLK_FREQUENCY;
    /*********比较各个时钟源的定时精度，选择最高精度的时钟源及定时时间**********/
    if ((smclk_OffectTime <= aclk_OffectTime)
            && (smclk_OffectTime <= vlo_OffectTime))
    {
        return (WDTSSEL__SMCLK | smclk_DividerValue);
    }
    else if ((aclk_OffectTime <= smclk_OffectTime)
            && (aclk_OffectTime <= vlo_OffectTime))
    {
