 ...
 *  使用示例：
 *************************************************************************/
void LCD_FillPart(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye,
                  uint16_t color_dat)
{
    uint16_t i, j;
 ...
 *  使用示例：
 *************************************************************************/
void LCD_DispChar(uint16_t x, uint16_t y, char c_dat, uint16_t word_color,
                  uint16_t back_color)
{
    if (c_dat < ' ' || c_dat > '~')
        return;
 ...
 *  使用示例：
 *************************************************************************/
void LCD_DispStringLen(uint16_t x, uint16_t y, char *s_dat, int num,
                       uint16_t word_color, uint16_t back_color)
{
    while (num--)
    {
 ...
 *  使用示例：
 *************************************************************************/
void LCD_DispStringAt(uint16_t x, uint16_t y, char *s_dat, uint16_t word_color,
                      uint16_t back_color)
{
    while (*s_dat != '\0')
    {
 ...
 *  使用示例：
 *************************************************************************/
void LCD_DispDecAt(uint16_t x, uint16_t y, int number, uint16_t word_color,
                   uint16_t back_color)
{
    char str[7], j, k = 0, i;
 ...
 *  使用示例：
 *************************************************************************/
void LCD_DispFolatAt(uint16_t x, uint16_t y, float number, int decimals,
                     uint16_t word_color, uint16_t back_color)
{
 ...
 *  使用示例：
 *************************************************************************/
void LCD_Show_Picture(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye,
                      uint16_t pic[])
{
...
void LCD_Printf_8x16(uint16_t x, uint16_t y, const char *fmt, ...)
{
    ...
    for (int i = 0; i <= length; i++)
    {
        if (LCD_String[i] == '\n')
        {
            ...
            while (position > lastposition)
            {
                ...
                {
                    LCD_DispStringLen(x, y, &LCD_String[lastposition],
                                      TEXT_SIZE, POINT_COLOR, BACK_COLOR);
                    y += 16;		//换行
                    x = 0;
                ...
                {
                    LCD_DispStringLen(x, y, &LCD_String[lastposition],
                                      (position - lastposition), POINT_COLOR,
                                      BACK_COLOR);
    ...
    if (lastposition < length)
        LCD_DispStringLen(x, y, &LCD_String[lastposition],
                          (length - lastposition), POINT_COLOR, BACK_COLOR);
    va_end(ap);
    free(LCD_String);
}
/*************************************************************************
