...
void KEY_Init(KEYn key, void (*ClickedCallBack)(KEY_EVENT))
{
    ...
    KeyStruct[key].Stage = KEY_RELEASE;    //按键处于松开状态
...
void KEY_20msTimingProcess(KEYn key)
{
    ...
    case KEY_PRESS_WOBBLE:
        ...
        {
            KeyStruct[key].Stage = KEY_RELEASE;          //如果消抖时间内松开则认为该次按键按下无效
        }
        break;
    case KEY_PRESSED:
        ...
        else
        {
            ...
            {
                KeyStruct[key].Stage = KEY_RELEASE_WOBBLE; //进入按键松开后消抖阶段
                KeyStruct[key].WobbleTime = 0;            //消抖时间清零，一边后续阶段消抖
            }
        }
        break;
    case KEY_RELEASE_WOBBLE:
        KeyStruct[key].WobbleTime += KEY_TIMING_PROCESS_TIME;     //按键松开消抖时间开始计时
        if (KEY_isPressed(key) == FALSE) //如果按键松开
        {
            if (KeyStruct[key].WobbleTime >= KEY_RELEASE_WOBBLE_TIME)  //消抖时间达到
            {
                KeyStruct[key].Stage = KEY_RELEASE;              //进入按键完全松开阶段
