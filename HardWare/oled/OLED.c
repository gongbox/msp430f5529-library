...
void OLED_DispChar(OLED_FONT font, uint8_t line, uint8_t x, char ascii)
{
    ...
    //读取存放字体数据的起始位置
    pFontData = Font[font].pData
            + (Font[font].width * Font[font].height / 8) * offect;
 ...
 *  使用示例：
 ********************************************************************************/
void OLED_DispStringLen(OLED_FONT font, uint8_t line, uint8_t x, char *str,
                        uint8_t len)
{
    while (len--)
    {
        ...
        if (x > (OLED_WIDTH - Font[font].width))
        {
            ...
            if (line > ((OLED_HEIGHT / Font[font].height) - 1))
                line = 0;
        }
    }
}
...
void OLED_DispChineseAt(OLED_FONT font, uint8_t line, uint8_t x, char *str)
{
    ...
    while (*str)
    {
        ...
        {
            if (*str == *FONT_CHINESE_TABLE[offect]
                    && *(str + 1) == *(FONT_CHINESE_TABLE[offect] + 1))
            {
                //读取存放字体数据的起始位置
                pFontData = Font[font].pData
                        + (Font[font].width * Font[font].height / 8) * offect;
                   ...
                   ...)
{
    ...
    for (int i = 0; i <= length; i++)
    {
        if (OLED_String[i] == '\n')
        {
            ...
            while (position > lastposition)
            {
                ...
                {
                    OLED_DispStringLen(font, line, x,
                                       &OLED_String[lastposition], TEXT_SIZE);
                ...
                {
                    OLED_DispStringLen(font, line, x,
                                       &OLED_String[lastposition],
                                       (position - lastposition));
                    for (x += (position - lastposition) * Font[font].width;
                            x < OLED_WIDTH; x++)   //将该行未显示的部分清零
                    {
    ...
    {
        OLED_DispStringLen(font, line, x, &OLED_String[lastposition],
                           (length - lastposition));
    }
    va_end(ap);
}
    ...
    for (int i = 0; i <= length; i++)  //查找换行符
    {
        if (OLED_String[i] == '\n')
        {
            ...
            while (position > lastposition)
            {
                ...
                {
                    OLED_DispStringLen(font, y, x, &OLED_String[lastposition],
                                       TEXT_SIZE);
